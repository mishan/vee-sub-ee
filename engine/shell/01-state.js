...
export class SafeHtml { constructor(s) { this.value = s; } toString() { return this.value; } }
export function raw(s) { return new SafeHtml(s == null ? '' : String(s)); }   // opt out: trust this markup
...
export let msgTimer = null, lastMsg = '';
...
export const TEST_MODE = ['new', 'syst', 'ship', 'x', 'y', 'heading', 'ff', 'land',
...
export const Save = {
  ...
  capture(spobId) {
    return {
      v: 1, syst: S.SYSTEM_ID, spob: spobId, ship: S.playerShipId,
...
export const PRICE_MULT = { low: 0.80, medium: 1.00, high: 1.25 };
S.credits = SAVED ? SAVED.credits : 10000;
export const cargo = Object.fromEntries(COMMODITIES.map(c => [c, (SAVED && SAVED.cargo[c]) || 0]));
...
export function newPilot() { // abandon the current pilot → back to the title
  Save.clear();
  location.href = location.pathname;
}
/* ---- New Pilot creation (spec: "New pilot") ---- */
export const nameSuggest = kind => { // a random default, from the app's STR# 128 or a generic
...
export const shipLongName = () => (DATA.strings[5002] && DATA.strings[5002].list[S.playerShipId - 128])
...
export const SHIP_SPIN_BASE = 128, STELLAR_SPIN_BASE = 300;
