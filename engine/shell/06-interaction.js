...
import { acceptMission, descText, dudeShipPairs, getOffer, govts, misns, onMissionShipDestroyed, pers, spobById, stelName, subst } from './08-missions.js';
...
export function cyclePlanetTarget() {
  if (!S.spobs.length) { showMsg('No stellar objects in this system.'); return; }
...
export function openHail(kind, obj) {
  ...
  hailOpen = true; S.hailTarget = { kind, obj }; renderHail();
}
export function closeHail() {
  clearTimeout(hailCloseTimer);
  hailOpen = false; S.hailTarget = null;
  document.getElementById('hail').style.display = 'none';
}
export function hailSay(text) { S.hailTarget.said = text; renderHail(); }

// Defense fleet size per wave (DefCount>1000 encodes waves; last digit = ships/wave).
export const defWave = p => { const c = p.DefCount; return c > 1000 ? Math.max(1, c % 10) : Math.max(0, c); };
...
export function closeHailSoon() { clearTimeout(hailCloseTimer); hailCloseTimer = setTimeout(closeHail, 1400); }
...
export function payBribe(s) {
  hailClick();
  if (S.credits < S.hailTarget.bribe) { hailSay(pickFrom(3000, 60, 64) || 'You can’t afford it.'); return; }
...
export function renderHail() {
  const t = S.hailTarget; if (!t) return;
  ...
  if (t.kind === 'ship') {
    ...
    if (t.mode === 'fuel') {
      ...
      buttons =
        (offering ? `<button onclick="acceptShipMission(S.hailTarget.obj)">Accept mission</button>
        ...
        (s.hostile ? `<button onclick="begForMercy(S.hailTarget.obj)">Beg for mercy</button>` : '') +
