...
(function (g) {
  ...
  class B extends Uint8Array {
    ...
    static from(x, enc) {
      if (typeof x === 'string') {
        if (enc === 'hex') {
          const a = new B(x.length >> 1);
          for (let i = 0; i < a.length; i++) a[i] = parseInt(x.substr(i * 2, 2), 16);
          return a;
        }
        const a = new B(x.length);
        for (let i = 0; i < x.length; i++) a[i] = x.charCodeAt(i) & 0xff;
        return a;
      }
      const a = new B(x.length);
      a.set(x);
    ...
    readInt16BE(o) {
      return (((this[o] << 8) | this[o + 1]) << 16) >> 16;
    }
    readUInt32BE(o) {
    ...
    equals(o) {
      if (!o || o.length !== this.length) return false;
