...
const refRGBA = (f) =>
  new Uint8Array(cp.execFileSync('convert', [f, '-depth', '8', 'RGBA:-'], { maxBuffer: 1 << 28 }));
...
function refPng(id) {
  for (const dir of ['graphics', 'titles']) {
    ...
    const hit = fs
      .readdirSync(A(dir))
      .find((f) => f === `PICT_${id}.png` || f.startsWith(`PICT_${id}_`));
...
function checkPict() {
  ...
  for (const file of ['EV Titles.rsrc', 'EV Graphics.rsrc']) {
    ...
    for (const r of pictResources(file)) {
      ...
      if (img.unhandled) {
        unhandled++;
        continue;
      } // decoder hit an opcode it doesn't model
      const rp = refPng(r.id);
      if (!rp) continue;
      tested++;
      ...
      for (let i = 0; ok && i < img.rgba.length; i += 4)
        if (
          img.rgba[i] !== ref[i] ||
          img.rgba[i + 1] !== ref[i + 1] ||
          img.rgba[i + 2] !== ref[i + 2]
        )
          ok = false;
      ok ? exact++ : mismatch++;
    }
  ...
  console.log(
    `PICT: ${exact}/${tested} pixel-exact (${pct(exact, tested)})  ` +
...
function checkSnd() {
  ...
  const snds = t.find((x) => x.typeName === 'snd ');
  if (!snds) return;
  let exact = 0,
    tested = 0;
  for (const r of snds.resources) {
    const hit = fs
      .readdirSync(A('sounds'))
      .find((f) => f === `snd_${r.id}.wav` || f.startsWith(`snd_${r.id}_`));
    if (!hit) continue;
    const wav = readWav(A('sounds', hit)),
      dec = decodeSnd(r.data());
    if (dec.error) continue;
    tested++;
    const ok =
      dec.sampleRate === wav.rate &&
      dec.pcm8 &&
      wav.bits === 8 &&
      dec.pcm8.length === wav.data.length &&
      dec.pcm8.every((v, i) => v === wav.data[i]);
    if (ok) exact++;
  }
...
function checkSprites() {
  ...
  const spins = t.find((x) => x.typeName === 'spïn');
  if (!spins) return;
  const picts = {};
  for (const r of t.find((x) => x.typeName === 'PICT').resources) picts[r.id] = r;
  let exact = 0,
    near = 0,
    tested = 0;
  for (const r of spins.resources) {
    const rec = decodeRecord(r.data(), schema),
      sp = picts[rec.SpritesID];
    const refp = A('sprites', `spin_${r.id}.png`);
    if (!sp || !fs.existsSync(refp)) continue;
    const comp = compositeSprite(
      decodePict(sp.data()),
      picts[rec.MasksID] ? decodePict(picts[rec.MasksID].data()) : null,
    );
    tested++;
    const ref = refRGBA(refp);
    const tot = comp.width * comp.height;
    let ok = ref.length === comp.rgba.length,
      m = 0;
    ...
    for (let i = 0; i < tot; i++) {
      ...
      if (
        comp.rgba[o] === ref[o] &&
        comp.rgba[o + 1] === ref[o + 1] &&
        comp.rgba[o + 2] === ref[o + 2] &&
        comp.rgba[o + 3] === ref[o + 3]
      )
        m++;
    }
    if (ok) exact++;
    else if ((100 * m) / tot >= 99) near++;
  }
  console.log(
    `sprite: ${exact}/${tested} byte-exact (${pct(exact, tested)}), ${exact + near}/${tested} ≥99% [edge alpha]`,
  );
}
...
function checkSit() {
  ...
  for (const e of entries) {
    ...
    tested++;
    const out = extractFork(sit, e),
      fork = loadFork(D(want[e.name])).fork;
...
function checkHardening() {
  ...
  const directBits = (bBot, bRight, pixelSize, cmpCount) => {
    const { b, dv } = pictBase();
    let p = 14;
    const w16 = (v) => {
        dv.setUint16(p, v);
  ...
  must(
    'decodePict throws on huge frame',
    () => {
      const { b, dv } = pictBase();
      dv.setInt16(6, 30000);
      dv.setInt16(8, 30000);
  ...
  must(
    'decodePict throws on direct-bits huge bounds',
    () => decodePict(directBits(30000, 30000, 16, 3)),
    true,
  );
  ...
  must('decodeSnd clamps a giant ext length', () => {
    const d = decodeSnd(snd16(0x7fffffff));
    return !!d.pcm16 && d.pcm16.length <= 100;
  });
  // .sit: out-of-range and huge-namelength entries must throw.
  must(
    'extractFork throws on oob range',
    () =>
      extractFork(new Uint8Array(1000), { offset: 900, compLength: 500, length: 10, method: 13 }),
    true,
  );
  must(
    'extractFork throws on implausible length',
    () => extractFork(new Uint8Array(1000), { offset: 0, compLength: 10, length: 4e9, method: 13 }),
    true,
  );
...
function checkPluginMerge() {
  ...
  const tuned = Buffer.from(orig);
  tuned[2] ^= 0xff;
  tuned[3] ^= 0xff; // perturb one field
  ...
  let pass = 0,
    fail = 0;
  const ok = (n, c) => {
    c ? pass++ : (fail++, console.log('  ✗ ' + n));
  };
  ok(
    'override changes ship 128',
    JSON.stringify(plug.types.ship[128]) !== JSON.stringify(base.types.ship[128]),
  );
  ok('new ship 200 added', !!plug.types.ship[200] && !base.types.ship[200]);
  ok(
    'ship count grew by exactly 1',
    Object.keys(plug.types.ship).length === Object.keys(base.types.ship).length + 1,
  );
  // ship 200's bytes are a copy of the *original* 128, so it decodes identically (bar the name).
  const a = { ...plug.types.ship[200] },
    b = { ...base.types.ship[128] };
  delete a.name;
  delete b.name;
  ...
  ok(
    ...
    norm(buildData(dataFork, schemasByType)) === norm(buildData(dataFork, schemasByType, [])),
  );
...
function checkPluginAssets() {
  const gF = D('EV Graphics.rsrc'),
    tF = D('EV Titles.rsrc'),
    sF = D('EV Sounds.rsrc');
  if (!fs.existsSync(gF) || !fs.existsSync(tF)) return;
  const gfx = loadFork(gF).fork,
    titles = loadFork(tF).fork;
  ...
  ok(
    'title-override replaced the bytes',
    !!titlePict && Buffer.from(titlePict.data()).equals(Buffer.from([3, 4])),
  );
  ...
  const m = buildManifest(
    gfx,
    JSON.parse(fs.readFileSync(path.join(ROOT, 'schemas', 'spin.json'), 'utf8')),
    [plugin],
  );
