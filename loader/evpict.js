...
function decodePict(bytes) {
  ...
  for (let guard = 0; guard < 100000 && p < u8.length; guard++) {
    ...
    if (op === 0x009a || op === 0x009b) {
      renderDirectBits(op === 0x009b);
      continue;
    }
  ...
  function renderBits(packed, hasRegion) {
    ...
    if (isPixmap) {
      ...
      pixelSize = rd16();
      rd16();
      rd16(); // cmpCount, cmpSize
      rd32(); // planeBytes
      rd32();
      rd32(); // pmTable, pmReserved
      rd32(); // ctSeed
      rd16(); // ctFlags
    ...
    for (let y = 0; y < bh; y++) {
      if (unpacked) {
        for (let i = 0; i < rowBytes; i++) rowBuf[i] = u8[p++];
      } else {
  ...
  function renderDirectBits(_hasRegion) {
    ...
    rd32();
    rd32();
    rd32(); // planeBytes, pmTable, pmReserved (no color table)
    const src = rdRect(),
      dst = rdRect();
    rd16(); // srcRect, dstRect, mode
    ...
    for (let y = 0; y < bh; y++) {
      if (unpacked) {
        for (let i = 0; i < rowLen; i++) rowBuf[i] = u8[p++];
      } else {
        const count = rowBytes > 250 ? rd16() : u8[p++];
      ...
      for (let x = 0; x < bw; x++) {
        ...
        if (pixelSize === 16) {
          ...
          // 32-bit planar (skip the alpha plane if present)
          r = rowBuf[planeBase + x];
          g = rowBuf[planeBase + bw + x];
          b = rowBuf[planeBase + bw * 2 + x];
        }
        const o = (oy * fw + ox) * 4;
        out.rgba[o] = r;
        out.rgba[o + 1] = g;
...
function unpackBits(src, off, count, out, outLen) {
  ...
  while (i < end && o < outLen) {
    ...
    if (n >= 0) {
      for (let k = 0; k <= n && o < outLen && i < end; k++) out[o++] = src[i++];
    } else if (n !== -128) {
      const b = src[i++];
      for (let k = 0; k < 1 - n && o < outLen; k++) out[o++] = b;
    }
  }
