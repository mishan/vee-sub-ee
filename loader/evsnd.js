...
function decodeSnd(bytes) {
  ...
  for (let i = 0; i < nCmd; i++) {
    const cmd = rd16();
    rd16();
    const param2 = rd32(); // cmd, param1, param2
  ...
  if (encode === 0xff) {
    ...
    rd32();
    rd32();
    rd32(); // markerChunk, instrumentChunks, AESRecording
...
function toFloat32(dec) {
  if (dec.pcm16) {
    const f = new Float32Array(dec.pcm16.length);
    for (let i = 0; i < f.length; i++) f[i] = dec.pcm16[i] / 32768;
    return f;
  }
  if (dec.pcm8) {
    const f = new Float32Array(dec.pcm8.length);
    for (let i = 0; i < f.length; i++) f[i] = (dec.pcm8[i] - 128) / 128;
    return f;
  }
