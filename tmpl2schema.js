...
const CODE_MAP = {
  DBYT: ['i8'], UBYT: ['u8'], HBYT: ['u8'],
  DWRD: ['i16'], UWRD: ['u16'], HWRD: ['u16'],
  DLNG: ['i32'], ULNG: ['u32'], HLNG: ['u32'],
  RECT: ['i16', 4],
  CSTR: ['cstr'],
};
...
function main() {
  ...
  const file = args[0];
  if (!file) { console.error('usage: tmpl2schema.js <datafile> [-o outdir]'); process.exit(1); }
  ...
  for (const r of tmpl.resources) {
    if (!r.name) { console.warn(`TMPL ${r.id}: unnamed, skipped`); continue; }
    const fields = parseTmpl(r.data());
    const schema = toSchema(r.name, fields,
      `TMPL ${r.id} ("${r.name}") in ${path.basename(file)}`);
    const dest = path.join(outDir, `${asciiAlias(r.name)}.json`);
    fs.writeFileSync(dest, JSON.stringify(schema, null, 2) + '\n');
    const fixed = schema.fields.reduce((n, [, t, rep]) =>
      n + (t === 'cstr' ? 0 : ({ i8: 1, u8: 1, i16: 2, u16: 2, i32: 4, u32: 4 }[t]) * (rep || 1)), 0);
    console.log(`${dest}  ${schema.fields.length} fields, ` +
